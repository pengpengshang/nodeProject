...
(function (selectaddress) {
    ...
    selectaddress.getElement = getElement;
    function getItems(str) {
        return dsy.Items[str];
    }
    selectaddress.getItems = getItems;
    ...
    selectaddress.indexof = indexof;
    function selectprovince(obj, provinceid, cityid, areaid) {
        var ddlProvince = getElement(provinceid);
        var ddlCity = getElement(cityid);
        var indexformprovince = indexof(dsy.Items["0"], ddlProvince.value); //找出第一列的下标
        var ar = dsy.Items["0_" + indexformprovince];
        if (obj == ddlProvince) {
        ...
        if (areaid != null) {
            var ddlarea = getElement(areaid);
            ddlarea.options.length = 0; //clear
            var indexformcity = indexof(dsy.Items["0_" + indexformprovince], ddlCity.value); //找出第二列的下标
            ar = dsy.Items["0_" + indexformprovince + "_" + indexformcity];
            addlist(ar, ddlarea);
        }
    }
    selectaddress.selectprovince = selectprovince;
    function addlist(ar, obj) {
        if (!ar)
            return;
        for (var i = 0; i < ar.length; i++) {
            var option = document.createElement("option");
            option.appendChild(document.createTextNode(ar[i]));
            option.value = ar[i];
            obj.appendChild(option);
        }
    }
    selectaddress.addlist = addlist;
