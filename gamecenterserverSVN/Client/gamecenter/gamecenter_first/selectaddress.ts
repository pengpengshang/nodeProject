...
module selectaddress {
    ...
    export function getElement(elementid) {
        var s: HTMLSelectElement = <HTMLSelectElement>document.getElementById(elementid);
        return s
    }
    export function getItems(str) {
        return dsy.Items[str];
    }
    export function indexof(obj, value) {
        var k = 0;
        for (; k < obj.length; k++) {
            if (obj[k] == value)
                return k;
    ...
    export function selectprovince(obj, provinceid, cityid, areaid) {
    ...
    export function addlist(ar, obj) {
        if (!ar) return;
