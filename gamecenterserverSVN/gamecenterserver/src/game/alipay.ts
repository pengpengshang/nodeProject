...
import {g_myServerDomain, g_serverport, HttpRequest, HttpsRequest} from "./gameapi";
/**
...
function getContentBytes(str:string,input_charset:string):any
{
...
export class MD5 {
     ...
     * @return 签名结果
     */
    public static sign( text:string,  key:string,  input_charset:string) :string
    {
        text = text + key;
     ...
     * @return 签名结果
     */
    public static  verify( text:string,  sign:string,  key:string,  input_charset:string):boolean {
        text = text + key;
...
class PAYPARA
{
    ...
    notify_url:string="http://"+g_myServerDomain+":"+g_serverport+"/alinotify";//支付宝服务器主动通知商户网站里指定的页面http路径
    out_trade_no:string="";//支付宝合作商户网站唯一订单号。
    partner:string=partner;//签约的支付宝账号对应的支付宝唯一用户号。以2088开头的16位纯数字组成。
    seller_id:string=partner;//卖家支付宝账号对应的支付宝唯一用户号。以2088开头的纯16位数字
    payment_type=1;
    return_url:string="";//支付宝处理完请求后，当前页面自动跳转到商户网站里指定页面的http路径。
    service:string="alipay.wap.create.direct.pay.by.user";//接口名称
...
export function GetSign(para:any,key:string=md5key):string//生成签名
{
    var keys:string[]=[];
    for(var k in para)
    {
...
export function CheckNotifyID(notify_id:string,cb:(success:boolean)=>void)
{
//    var str="http://notify.alipay.com/trade/notify_query.do?partner="+ partner+"&notify_id="+notify_id;
    ...
    HttpsRequest(str,false,null,(data)=>{
        data=data.trim();
        cb(data=="true");
    });
}
...
//返回支付宝链接
export function  Pay(out_trade_no:string,subject:string,total_fee:string,show_url:string,return_url?:string,notify_url?:string):string
{
    var sign:string;//签名
    ...
    if(notify_url)para.notify_url=notify_url;
